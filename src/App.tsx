...
function F1GuideModal({session,onChange,onClose}:{session:FlightSession;onChange:(next:FlightSession)=>void;onClose:()=>void}){
 ...
 const tailTypeMap=Object.fromEntries([['32LD','A319-153N'],['32M1','A319-153N'],['32NG','A319-153N'],['32NH','A319-153N'],['6941','A320-214'],['6967','A320-214'],['9918','A320-214'],['9923','A320-214'],['9925','A320-214'],['9926','A320-214'],['328P','A320-251N'],['326W','A320-251N'],['324H','A320-251N'],['324T','A320-251N'],['321K','A320-251N'],['322K','A320-251N'],['324J','A320-251N'],['322W','A320-251N'],['325Y','A320-251N'],['326A','A320-251N'],['325Z','A320-251N'],['326V','A320-251N'],['32A3','A320-251N'],['32AQ','A320-251N'],['32AR','A320-251N'],['8891','A320-271N'],['8890','A320-271N'],['1068','A320-271N'],['1050','A320-271N'],['301F','A320-271N'],['305K','A320-271N'],['305J','A320-271N'],['308N','A320-271N'],['309H','A320-271N'],['309S','A320-271N'],['30A6','A320-271N'],['30AH','A320-271N'],['30C1','A320-271N'],['30C2','A320-271N'],['30DE','A320-271N'],['30DA','A320-271N'],['30C7','A320-271N'],['30DC','A320-271N'],['30EJ','A320-271N'],['30EK','A320-271N'],['321L','A320-271N'],['322X','A320-271N'],['32FG','A320-271N'],['32FH','A320-271N'],['32FJ','A320-271N'],['32FK','A320-271N'],['6361','A321-213'],['6362','A321-213'],['6365','A321-213'],['6382','A321-213'],['6386','A321-213'],['6556','A321-213'],['6599','A321-213'],['6603','A321-213'],['6631','A321-213'],['6632','A321-213'],['325A','A321-251NX'],['326X','A321-251NX'],['327T','A321-251NX'],['329V','A321-251NX'],['326C','A321-251NX'],['32CC','A321-251NX'],['32C9','A321-251NX'],['32A2','A321-251NX'],['305H','A321-271N'],['305G','A321-271N'],['30AG','A321-271N'],['30C6','A321-271N'],['326U','A321-272NX'],['327R','A321-272NX'],['30FY','A321-272NX'],['323Q','A321-272NX'],['327S','A321-272NX']]) as Record<string,string>;
 const parseFlightText=(raw:string)=>{const text=raw||'',lines=text.split(/\r?\n/).map(x=>x.trim()).filter(Boolean);const codeName:Record<string,string>={TFU:'天府',CTU:'双流',YNT:'烟台'};const flt=text.match(/航班号\[FLT\]\s*([A-Z0-9/]+)/)?.[1]??'',act=text.match(/机型\[ACT\]\s*([A-Z0-9]+)/)?.[1]??'',arn=(text.match(/机号\[ARN\]\s*(B-[A-Z0-9]+)/i)?.[1]??'').toUpperCase();const std=text.match(/起飞时间\[STD\]\s*(\d{4}-\d{2}-\d{2})\s+(\d{2}:\d{2})/);const crew=lines.filter(l=>/^[\u4e00-\u9fa5]{2,4}(?:\s+\[[A-Z]+\])?\s+[AF]\d/.test(l)).map(l=>l.match(/^([\u4e00-\u9fa5]{2,4})/)?.[1]??'').filter(Boolean);const segs=[...text.matchAll(/[A-Z]{2}\d+\s+([A-Z]{3})-([A-Z]{3})/g)];const routeCodes=segs.reduce<string[]>((arr,m)=>{const a=m[1],b=m[2];if(arr.at(-1)!==a)arr.push(a);arr.push(b);return arr},[]);const route=routeCodes.length?routeCodes.map(c=>codeName[c]??c).join('-'):(text.match(/航线\[AIRLINE\]\s*([^\n]+)/)?.[1]??'').trim();const tail=arn.replace('B-',''),model=tailTypeMap[tail]??(act?`系统机型代码 ${act}`:'——');return{flt,act,arn,stdDate:std?.[1]??'',stdClock:std?.[2]??'',crew,route,model}};
