...
import{AlertTriangle,ArrowLeft,ArrowRight,BookOpen,Calculator,Check,ChevronRight,CloudSun,Download,Edit3,Plane,LayoutGrid,ListChecks,Plus,RotateCcw,Search,Settings2,ShieldAlert,Trash2,Upload,X}from'lucide-react';
...
export default function App(){
 ...
 return <div className="app-shell">
  ...
  <main className="workspace"><header className="topbar"><div className="segmented"><button className={view==='current'?'active':''} onClick={()=>setView('current')}><ListChecks/>当前阶段</button><button className={view==='overview'?'active':''} onClick={()=>setView('overview')}><LayoutGrid/>总览</button></div><div className="top-actions"><button onClick={()=>setModal('f1guide')}><BookOpen/>F1跟班</button><button onClick={()=>setModal('duty')}><Calculator/>值勤计算</button><button onClick={()=>setModal('environment')}><CloudSun/>环境条件{matched.length>0&&<b>{matched.length}</b>}</button><button onClick={()=>setModal('aircraft')}><Plane/>飞机信息</button><button onClick={()=>setModal('subjects')}><ShieldAlert/>模拟机科目{session.activeSubjects.filter(x=>x.status==='active').length>0&&<b>{session.activeSubjects.filter(x=>x.status==='active').length}</b>}</button><button className="icon-btn" onClick={()=>setModal('settings')}><Settings2/></button></div></header>
