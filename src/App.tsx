...
export default function App(){
 const[loading,setLoading]=useState(true),[session,setSession]=useState<FlightSession>(newSession()),[rules,setRules]=useState<EnvironmentRule[]>([]),[subjects,setSubjects]=useState<TrainingSubject[]>([]),[base,setBase]=useState<PhaseOutput[]>(emptyBaseOutputs),[view,setView]=useState<'current'|'overview'>('current'),[modal,setModal]=useState<string|null>(null),[showIncomplete,setShowIncomplete]=useState(false);const importRef=useRef<HTMLInputElement>(null);
 useEffect(()=>{(async()=>{await bootstrapData();setSession((await db.sessions.get('current'))!);setRules(await db.rules.toArray());setSubjects(await db.subjects.toArray());setBase((await db.settings.get('baseOutputs'))?.value??emptyBaseOutputs);setLoading(false)})()},[]);
